...
<div style="max-width:800px;margin:0 auto;">
  ...
  <div style="margin:0 40px 32px;background:#f0fdf4;border:1px solid #22c55e44;border-radius:10px;padding:24px;text-align:center;">
    <div style="font-size:18px;font-weight:800;margin-bottom:8px;color:#0b1628;">Get Your Full Professional Report — $97</div>
    <div style="font-size:13px;color:#64748b;margin-bottom:12px;line-height:1.6;">
      Step-by-step fix for every finding · Custom 90-day roadmap · Policy recommendations · CISSP-certified
    </div>
    <div style="font-size:13px;color:#475569;">Visit <strong>greensecuritygroup.com</strong> to purchase</div>
  </div>
...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .report-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 20px; }
...
export default function App() {
  ...
  const makeFullReport = () => {
    ...
    const month1 = top10.filter(q => priorityFor(q) === "CRITICAL").slice(0,4);
    const month2 = top10.filter(q => priorityFor(q) === "HIGH").slice(0,4);
    const month3 = top10.filter(q => priorityFor(q) === "MEDIUM").slice(0,4);

    const roadmapSection = (items, title, color) => {
      ...
      ${items.map((q,i) => {
        ...
        if (!rem) return "";
        return `<tr style="background:${i%2===0?"#f8fafc":"#fff"}">
          <td style="padding:8px 12px;font-size:11px;">Week ${i+1}</td>
...
${roadmapSection(month1, "Month 1 — Critical Priority Fixes", "#ef4444")}
${roadmapSection(month2.length ? month2 : month1.slice(0,2), "Month 2 — High Priority Improvements", "#f59e0b")}
${roadmapSection(month3.length ? month3 : month2.slice(0,2), "Month 3 — Medium Priority & Maintenance", "#22c55e")}
...
<div style="margin-top:24px;padding:22px 28px;background:#0b1628;border-radius:10px;text-align:center;">
  ...
  <div style="font-size:12px;color:#64748b;">Visit <strong style="color:#22c55e;">greensecuritygroup.com</strong></div>
</div>
    ...
    <div>
      ...
      {phase === "results" && (
        <div className="results">
          ...
          {/* Report cards */}
          <div style={{ fontWeight: 800, fontSize: 16, marginBottom: 12 }}>Get Your Report</div>
