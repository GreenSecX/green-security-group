...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(max-width:640px){
    ...
    .nav{padding:0 1.25rem;} .about-hero,.about-cta{padding-left:1.25rem;padding-right:1.25rem;}
    .about-body{padding:2rem 1.25rem;} .about-cta-inner{flex-direction:column;align-items:flex-start;}
    .footer-inner{flex-direction:column;align-items:flex-start;} .footer{padding:2.5rem 1.25rem;}
    .dom-name{width:130px;} .ass-wrap,.res-wrap{padding:1.5rem 1.25rem;}
    .form-grid{grid-template-columns:1fr;} .hero h1{font-size:2.2rem;}
...
const ASS_REV = [
  { name:"Mark T.",  role:"Owner, Regional Accounting Firm",   rating:5, text:"I was expecting something technical and confusing. Instead I understood every question and actually felt empowered to act on the results. Shared the report with my IT vendor the same day." },
  { name:"Sarah K.", role:"Practice Manager, Family Dental Clinic", rating:5, text:"As someone with no IT background, I appreciated how clearly everything was explained. We found three gaps we had no idea existed. Worth every minute." },
  { name:"David R.", role:"Operations Director, Food Distribution", rating:4, text:"The report gave me something concrete to bring to our IT provider. Instead of vague security conversations we had a specific prioritized list. Very practical." },
];
const TK_REV = [
  { name:"Jennifer M.", role:"Owner, Physiotherapy Practice",   rating:5, text:"The policy templates were ready to use in under an hour. Our insurance broker was impressed we already had these in place." },
  { name:"Carlos B.",   role:"General Manager, Regional Credit Union", rating:5, text:"The Risk Tracking Spreadsheet is exactly what I needed to report to our board. Professional, clear, and easy to update." },
  { name:"Priya N.",    role:"COO, E-commerce Business",        rating:4, text:"The 90-Day Action Plan removed all the guesswork. We knew exactly what to do and in what order. Three months later we feel genuinely more secure." },
];
...
function ReviewSection({ seed }) {
  ...
  return (
    <div className="reviews">
      <div className="rev-header"><div className="rev-label">What people are saying</div><div style={{fontSize:"0.78rem",color:"var(--muted)"}}>{reviews.length} review{reviews.length!==1?"s":""}</div></div>
      <div className="rev-cards">{reviews.map((r,i)=>(
        <div key={i} className="rev-card"><Stars n={r.rating}/><p className="rev-text">&#8220;{r.text}&#8221;</p><div className="rev-author">{r.name}</div><div className="rev-role">{r.role}</div></div>
      ))}</div>
      <div className="rev-form">
        <div className="rev-form-title">Share your experience</div>
        {ok?<div className="form-ok">&#10003; Thank you for your review!</div>:<>
          <div className="form-grid">
            <div className="field"><label>Your Name</label><input type="text" placeholder="Jane Smith" value={form.name} onChange={e=>upd("name",e.target.value)}/></div>
            <div className="field"><label>Your Role / Business (optional)</label><input type="text" placeholder="e.g. Owner, 12-person accounting firm" value={form.role} onChange={e=>upd("role",e.target.value)}/></div>
            <div className="field"><label>Rating</label><select value={form.rating} onChange={e=>upd("rating",e.target.value)}>
            ...
            <div className="field full"><label>Your Review</label><textarea placeholder="How did you use this? What did you find most helpful?" value={form.text} onChange={e=>upd("text",e.target.value)}/></div>
          </div>
    ...
    <div>
      ...
      <section className="section" id="projects">
        <div className="section-inner">
          ...
          <p className="section-sub">Two free projects designed to help small business owners understand and improve their security — without needing an IT background.</p>
