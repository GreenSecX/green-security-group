...
// Valid code prefix — Gumroad codes start with GSG-
const isValidCode = (code) => code.trim().toUpperCase().startsWith("GSG-") && code.trim().length >= 8;
...
function generateFreeReport(company, answers, scorePct, risk) {
  ...
  <div style="margin:0 40px 40px;background:linear-gradient(135deg,rgba(34,197,94,0.08),rgba(34,197,94,0.03));border:1px solid rgba(34,197,94,0.2);border-radius:12px;padding:32px;text-align:center;">
    ...
    <div style="font-size:13px;color:#64748b;">Visit <strong>greensecuritygroup.com</strong> · Enter your Gumroad unlock code · Get your report</div>
  </div>
...
export default function App() {
  ...
  const restart = () => { setPhase("hero"); setAnswers({}); setSec(0); setCompany(""); setAiReport(""); setEmail(""); setCode(""); setCodeError(""); };
  ...
  const handleCodeSubmit = async () => {
    setCodeError("");
    if (!email || !email.includes("@")) { setCodeError("Please enter a valid email address."); return; }
    if (!isValidCode(code)) { setCodeError("Invalid unlock code. Codes start with GSG- and are found in your Gumroad receipt email."); return; }
    setShowModal(false);
    setGenerating(true);
    setGenStep(0);
    ...
    try {
      ...
      setGenerating(false);
      alert("Report generation failed. Please email paul@greensecuritygroup.com with your unlock code and we will send your report manually.");
    }
  };
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="modal">
            ...
            <div className="modal-title">Enter Your Unlock Code</div>
            <div className="modal-sub">
              Your unlock code is in your Gumroad receipt email. Don't have one yet? Purchase the risk report below — you'll receive your code within minutes.
            </div>

            {/* Buy on Gumroad link */}
            <div className="modal-gumroad-link" onClick={()=>window.open("https://paulgreen.gumroad.com/l/ai-security-report","_blank")}>
              <span style={{fontSize:20}}>🛒</span>
              <div>
                <div className="modal-gumroad-text">Purchase Risk Report — $97</div>
                <div className="modal-gumroad-sub">Opens Gumroad · Receive unlock code by email</div>
              </div>
            ...
            <label className="modal-label">Unlock Code (from Gumroad receipt)</label>
            <input className={`modal-input ${codeError?"error":""}`}
              placeholder="GSG-12345"
              value={code} onChange={e=>{setCode(e.target.value);setCodeError("");}}
              onKeyDown={e=>e.key==="Enter"&&handleCodeSubmit()} />
            {codeError && <div className="modal-error">{codeError}</div>}

            <div style={{fontSize:11,color:"var(--dim)",fontFamily:"var(--mono)",marginBottom:20,lineHeight:1.6}}>
              Your code starts with GSG- followed by your order number.<br/>
              Check your Gumroad receipt email if you can't find it.
            </div>

            <div className="modal-btns">
              <button className="modal-cancel" onClick={()=>setShowModal(false)}>Cancel</button>
              <button className="modal-confirm" onClick={handleCodeSubmit}
                disabled={!email.includes("@")||code.length<4}>
                Generate My Report →
              </button>
            </div>
          </div>
        ...
        <div className="features">
          ...
          <div className="feat-grid">
            {[
              ...
              ["📋","Step 4: Risk Report ($97)","Purchase on Gumroad, enter your unlock code, get a personalized 15-page risk report in 60 seconds."],
              ["🛡️","CISSP-Certified","Every report built on frameworks by Paul Green — 13 certifications, 10+ years enterprise experience."],
              ["⚡","Instant Delivery","Free report downloads immediately. risk report generated in under 60 seconds after code entry."],
            ].map(([icon,name,desc])=>(
        ...
        <div className="results">
          ...
          <div className="report-cards">
            ...
            <div className="report-card rc-paid">
              ...
              <div className="rc-desc">A 15-page report written specifically for your business specifically from your answers. Purchase on Gumroad, enter your unlock code, get your report in 60 seconds.</div>
              <div className="rc-includes">
              ...
              <button className="rc-btn rc-btn-paid" onClick={()=>setShowModal(true)}>I Have an Unlock Code →</button>
              <div style={{textAlign:"center",marginTop:10}}>
                <span style={{fontSize:11,color:"var(--muted)",cursor:"pointer",textDecoration:"underline"}}
                  onClick={()=>window.open("https://paulgreen.gumroad.com/l/ai-security-report","_blank")}>
                  Don't have a code? Purchase on Gumroad →
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="results">
          ...
          <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:14,padding:28,textAlign:"center",marginBottom:20}}>
            ...
            <button className="rc-btn rc-btn-paid" style={{maxWidth:280,margin:"0 auto",display:"block"}}
              onClick={()=>window.open("https://gumroad.com","_blank")}>Get the Full Toolkit — $67 →</button>
          </div>
          <button className="restart-btn" onClick={restart}>↺ Start New Assessment</button>
        </div>
      )}
